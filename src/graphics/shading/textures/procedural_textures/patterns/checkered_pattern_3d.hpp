...
namespace gfx {
    class CheckeredPattern3D : public PatternTexture3D
    {
    public:
        /* Constructors */

        // Default Constructor
        CheckeredPattern3D() = delete;

        // Texture-Only Constructor
        CheckeredPattern3D(const Texture& texture_a, const Texture& texture_b)
                : PatternTexture3D{ texture_a, texture_b }
        {}

        // Color-Only Constructor
        CheckeredPattern3D(const Color& color_a, const Color& color_b)
                : PatternTexture3D{ color_a, color_b }
        {}

        // Standard Constructor (Textures)
        CheckeredPattern3D(const Matrix4& transform_matrix, const Texture& texture_a, const Texture& texture_b)
                : PatternTexture3D{ transform_matrix, texture_a, texture_b }
        {}

        // Standard Constructor (Colors)
        CheckeredPattern3D(const Matrix4& transform_matrix, const Color& color_a, const Color& color_b)
                : PatternTexture3D{ transform_matrix, color_a, color_b }
        {}

        /* Destructor */

        ~CheckeredPattern3D() override = default;

        /* Object Operations */

        [[nodiscard]] std::shared_ptr<Texture> clone() const override
        { return std::make_shared<CheckeredPattern3D>(*this); }
