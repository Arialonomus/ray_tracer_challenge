...
{
    const std::vector<float> matrix_a_vals{
            1.0, 5.0,
            -3.0, 2.0,
    };
    const float matrix_a_determinant_actual{ gfx::calculateDeterminant(matrix_a_vals) };
    const float matrix_a_determinant_expected{ 17 };
...
TEST(GraphicsLinearAlgebra, Determinant3x3)
{
    ...
    const float matrix_a_determinant_actual{ gfx::calculateDeterminant(matrix_a_vals) };
    const float matrix_a_determinant_expected{ -196 };
...
TEST(GraphicsLinearAlgebra, Determinant4x4)
{
    ...
    const float matrix_a_determinant_actual{ gfx::calculateDeterminant(matrix_a_vals) };
    const float matrix_a_determinant_expected{ -4071 };

    EXPECT_FLOAT_EQ(matrix_a_determinant_actual, matrix_a_determinant_expected);

    const std::vector<float> matrix_b_vals{
            6.0, 4.0, 4.0, 4.0,
            5.0, 5.0, 7.0, 6.0,
            4.0, -9.0, 3.0, -7.0,
            9.0, 1.0, 7.0, -6.0
    };
    const float matrix_b_determinant_actual{ gfx::calculateDeterminant(matrix_b_vals) };
    const float matrix_b_determinant_expected{ -2120 };

    EXPECT_FLOAT_EQ(matrix_b_determinant_actual, matrix_b_determinant_expected);

    const std::vector<float> matrix_c_vals{
            -4.0, 2.0, -2.0, -3.0,
            9.0, 6.0, 2.0, 6.0,
            0.0, -5.0, 1.0, -5.0,
            0.0, 0.0, 0.0, 0.0
    };
    const float matrix_c_determinant_actual{ gfx::calculateDeterminant(matrix_c_vals) };
    const float matrix_c_determinant_expected{ 0 };
